...
int main(int argc, char **argv) {
    ...
    while(1){
        ...
        bcm2835_i2c_setSlaveAddress(0x4d); // Temp sensor Address
        wbuf[0] = 0;
        data = bcm2835_i2c_write(wbuf, 1);
        data = bcm2835_i2c_read(buf_temp,1);
        bcm2835_i2c_setSlaveAddress(slave_address); // DS1307 Address
        buf[0] =  (buf[0] & 0x0F) + (((buf[0] >> 4)&0x07)* 10);
        buf[1] = (buf[1] >> 4)* 10 + buf[1] & 0x0F;

        
        if ((buf[0] % 10 == 0 && buf[0] != seg) || (buf_temp[0] >= 36 && buf_temp[0] != last_temp)){
            ...
            if (count >= 3){
                ...
                rename("./replica.txt", "./temperature.txt");
            }
            fptr = fopen("./temperature.txt","a");
            day = days[buf[3] - 1];
            fprintf(fptr,"Temperature: = %d ", buf_temp[0]);
            fprintf(fptr,"Record %d: %d/%d/%d %s %d:%d:%d AM\n",counter,buf[4],buf[5],buf[6],day, buf[2], buf[1], buf[0]);
            
            seg = buf[0];
            last_temp = buf_temp[0];
            counter++;
